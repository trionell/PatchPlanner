...
export interface BadgeProps extends HTMLAttributes<HTMLSpanElement> {
  /**
   * Accepts any vocabulary value; values without a dedicated style (e.g.
   * user-added signal or output types) fall back to the default look.
   */
  variant?: string
}

export function Badge({ className, variant, ...props }: BadgeProps) {
  const known = variant && variant in variantStyles ? (variant as keyof typeof variantStyles) : 'default'
  return <span className={cn(badgeVariants({ variant: known }), className)} {...props} />
}
